...
exports.updateLikesCount2 = functions.database.ref(`/accounts/{userId}/myLikes/{petId}`)
  .onCreate(event => {
    ...
    return admin.database().ref(`/pets/${petId}/likes`).transaction( likes => {
      if (likes) {
        likes = likes + 1;
      }
      return likes || 1;
...
exports.updateLikesCount = functions.database.ref(`/accounts/{userId}/myLikes/{petId}`)
  .onCreate(event => {
    ...
    return admin.database().ref(`/pets/${petId}/ownerUid`).once('value')
      .then(snapshot => {
        ...
        return admin.database().ref(`/accounts/${owner}/pets/${petId}/likes/`).transaction( likes => {
          if (likes) {
            likes = likes + 1;
          }
          return likes || 1;
...
exports.updateUnlikesCount = functions.database.ref(`/accounts/{pushId}/myLikes/{petId}`)
  .onDelete(event => {
    // Grab the current value of what was written to the Realtime Database.
    const petId = event.params.petId; //gets the petid from the {petId} url
    return admin.database().ref(`/pets/${petId}/likes/`).transaction( likes => {
...
exports.updateUnlikesCount2 = functions.database.ref(`/accounts/{pushId}/myLikes/{petId}`)
  .onDelete(event => {
  // Grab the current value of what was written to the Realtime Database.
    const petId = event.params.petId; //gets the petid from the {petId} url
    return admin.database().ref(`/pets/${petId}/ownerUid`).once('value')
